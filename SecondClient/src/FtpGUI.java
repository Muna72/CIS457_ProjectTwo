...
import java.awt.event.WindowEvent;
    ...
    public class FtpGUI extends JFrame implements ActionListener {
        ...
        public FtpGUI() {
            ...
            position.insets =  new Insets(0, 0, 0, 320);
            add(commandArea,position);

            cmdLine = new JPanel();
            cmdLine.setPreferredSize(new Dimension(600,200));
            cmdLine.setLayout(new GridBagLayout());
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            if (e.getSource() == go) {
                try {
                    stringsToDisplay.add(">> " + command.getText());
            ...
            if (e.getSource() == connect) {
                ...
                !userName.getText().equals("") && !hostName.getText().equals("")) {
                    try {
                        ...
                        go.setEnabled(true);
                        connectionTextString = ">> Succesfully connected to IP " + serverHostName.getText() + " Port " + portNum.getText();
                        stringsToDisplay.add(connectionTextString);
                    } catch (Exception ex) {
                        ex.printStackTrace();
